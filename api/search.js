...
async function analyserUnHadith(r, idx, q) {
  ...
  try {
    ...
    // ── GARDE-FOU GRADE_INVERSION_DETECTEE ────────────────────────────────
    // Vérifie que l'IA n'a pas inversé le grade fourni par Dorar
    if (parsed && r.grade) {
      const dorarIsWeak   = /ضعيف|منكر|موضوع|باطل|مكذوب/.test(r.grade);
      const dorarIsStrong = /صحيح|حسن/.test(r.grade);          // حسن = fort
      const expl = (parsed.grade_explique || "").toLowerCase();
      const claimsSahih = /\bsahih\b|\bhasan\b|#2ecc71|#f39c12/.test(expl);
      const claimsDaif  = /\bda.if\b|#e74c3c/.test(expl);

      if (dorarIsWeak && claimsSahih && !claimsDaif) {
        console.log("HADITH[" + idx + "]_GRADE_INVERSION_DETECTEE (Da'if→Sahih) — corrige");
        parsed.grade_explique = DEFAULTS.grade_explique;
      }
      if (dorarIsStrong && claimsDaif && !claimsSahih) {
        console.log("HADITH[" + idx + "]_GRADE_INVERSION_DETECTEE (Sahih→Da'if) — corrige");
        parsed.grade_explique = DEFAULTS.grade_explique;
      }
    }
