// ═══════════════════════════════════════════════════════════════════════════════
// MOTEUR MÎZÂN v16 — EDGE RUNTIME · SONNET 3.5 · STREAMING FOUDROYANT
// api/search.js
//
// NOUVEAUTÉS v16 :
//   ● export const config = { runtime: "edge" } — démarrage < 1s, zéro cold-start
//   ● Modèle : claude-3-5-sonnet-20240620 — qualité maximale
//   ● max_tokens : 1200 — flux rapide + JSON toujours fermé
//   ● SSE via Web Streams API (ReadableStream + TextEncoder)
//   ● Handler retourne Response (Edge-compatible, pas Node res.write)
//   ● URL.searchParams + req.json() — parsing requête Edge natif
//   ● Prompt TURBO : directive { en premier caractère absolu
//
...
// ═══════════════════════════════════════════════════════════════════════════════
// SYSTEM_TAKHRIJ v16 — 14 SIÈCLES · ANTI-TRONCATURE · 9 CHAMPS · TURBO
// ═══════════════════════════════════════════════════════════════════════════════
const SYSTEM_TAKHRIJ = `\
════════════════════════════════════════════════════════════
IDENTITE ET MISSION — MOTEUR MÎZÂN v16 TURBO
════════════════════════════════════════════════════════════
...
══════════════════════════════════════════════
ZONE 3 — TRESOR DES 14 SIECLES
══════════════════════════════════════════════
CONTRAINTE ABSOLUE ANTI-TRONCATURE :
Budget total avis_savants + grille_albani = MAX 600 tokens.
Chaque paragraphe = EXACTEMENT 2 phrases. Pas une de plus.
Style TELEGRAPHIQUE. Si tu depasses 2 phrases, tu INVALIDES le JSON.

CHAMP avis_savants : 7 PARAGRAPHES — 2 PHRASES CHACUN.

<span style='color:#d4af37;font-weight:bold;font-size:14px;'>❮ TRESOR DES 14 SIECLES ❯</span><br><br>

P1 <span style='color:#e8c96a;font-weight:bold;'>✹ AL-MANA AL-IJMALI — Ibn al-Athir</span><br>
2 phrases MAX : sens lexical An-Nihayah (Ibn al-Athir m.606H) + Asbab al-Wurud si connu.

P2 <span style='color:#a78bfa;font-weight:bold;'>✹ SAHABA ET TABI IN (7e-8e s.)</span><br>
2 phrases MAX : 1 avis Sahabi (Umar/Ali/Ibn Abbas/Aisha) + 1 Tabi i (Said m.94H ou Hasan Basri m.110H).

P3 <span style='color:#5dade2;font-weight:bold;'>✹ IMAMS FONDATEURS (8e-9e s.)</span><br>
2 phrases MAX : Malik (m.179H) ou Ahmad (m.241H) + divergence ecoles si existante.

P4 <span style='color:#f59e0b;font-weight:bold;'>✹ HUFFADH MEDIEVAUX (13e-15e s.)</span><br>
2 phrases MAX : Ibn Taymiyyah (m.728H) ou Ibn al-Qayyim (m.751H) + Ibn Hajar (m.852H) Fath al-Bari.

P5 <span style='color:#22c55e;font-weight:bold;'>✹ AL-AHKAM — Regles islamiques</span><br>
2 phrases MAX : statut juridique exact (Wajib/Mustahabb/Haram/Mubah) + conditions Fuqaha.

P6 <span style='color:#34d399;font-weight:bold;'>✹ AT-TATBIQ AL-AMALI — Application pratique</span><br>
2 phrases MAX : conduite si Sahih (agir) ou si Da if/Mawdu (ne pas citer).

P7 <span style='color:#f39c12;font-weight:bold;'>✹ CONTEMPORAINS — Al-Albani · Ibn Baz · Ibn Uthaymin</span><br>
2 phrases MAX : Al-Albani (SS/SD no., m.1420H) + Ibn Baz (m.1420H) ou Ibn Uthaymin (m.1421H).
Si Da if/Mawdu : <span style='color:#e74c3c;font-weight:bold;'>AVERTISSEMENT</span> — ne pas citer.

CHAMP grille_albani : 3 elements MAX separes par <br>.
1. Verdict Al-Albani + no.SS/SD.
2. Methode : rawis + shawahid.
3. Divergence notable si existante.
...
5.isnad_chain >= 8 maillons ? 6.grade_explique 4 lignes ? 7.Zone 3 : 7 paragraphes 2 phrases ?`;
...
async function analyserUnHadith(r, idx, q, ctrl, wantSSE) {
  const prompt =
    ...
    "  BUDGET : max 1200 tokens. JSON DOIT se fermer avant la limite.\n"+
    "  ANTI-TRONCATURE : Zone 3 = 2 phrases MAX par paragraphe.\n"+
    ...
    "  Zone 3 — 7 paragraphes — 2 PHRASES CHACUN.\n"+
