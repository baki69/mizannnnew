// ═══════════════════════════════════════════════════════════════════════════════
// MOTEUR MÎZÂN v18.6 — SERVERLESS NODE.JS · SONNET 3.5 (20241022) · DENSITÉ ABSOLUE · SCHOLAR-GLOW
// api/search.js
//
// NOUVEAUTÉS v18.6 :
//   ● Modèle : claude-3-5-sonnet-20241022 (ID exact — zéro alias)
//   ● Traducteur : claude-3-5-haiku-20241022 (ID exact — zéro alias)
//   ● URL : new URL() partout — zéro url.parse(), zéro encodeURIComponent brut
...
//   ● Prompt v18.6 : obligation scholar-glow dans SYSTEM_TAKHRIJ
//
...
//   ● Serverless Node.js 20 — handler (req, res) natif · res.write/res.end
// ═══════════════════════════════════════════════════════════════════════════════

import Anthropic from "@anthropic-ai/sdk";

// ── Serverless Node.js 20 — compatible @anthropic-ai/sdk · maxDuration 60s ────
export const maxDuration = 60;

const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });

// ═══════════════════════════════════════════════════════════════════════════════
// SYSTEM_TAKHRIJ v18.6 — 14 SIÈCLES · DENSITÉ ABSOLUE · SCHOLAR-GLOW · 9 CHAMPS
// ═══════════════════════════════════════════════════════════════════════════════
const SYSTEM_TAKHRIJ = `\
════════════════════════════════════════════════════════════
IDENTITE ET MISSION — MOTEUR MÎZÂN v18.6
════════════════════════════════════════════════════════════
...
async function _translateQueryToArabic(query) {
  ...
  try {
    const resp = await client.messages.create({
      model: "claude-3-5-haiku-20241022", max_tokens: 80, temperature: 0,
...
// ═══════════════════════════════════════════════════════════════════════════════
// SSE helpers — Serverless Node.js (res.write + res.flush)
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// analyserUnHadith — Streaming Node.js + Garde-fou
...
// ═══════════════════════════════════════════════════════════════════════════════
// _doDirectAnalysis — Analyse IA directe quand Dorar ne trouve rien
// ⚠️ Timeout-safe : délégue entièrement à analyserUnHadith
// ═══════════════════════════════════════════════════════════════════════════════
async function _doDirectAnalysis(q, arabicQuery, res) {
  const synth = {
    arabic_text: arabicQuery.substring(0,500) || q.substring(0,500),
    grade: "غير محدد",
    savant: "Non indexé dans Dorar.net — Analyse directe",
    source: "Analyse directe — requête utilisateur",
    rawi: ""
  };

  sseWrite(res, "dorar", [synth]);
  sseStatus(res, "TAKHRIJ");
  sseStatus(res, "RIJAL");

  const parsed = await analyserUnHadith(synth, 0, q, res, true);
  sseStatus(res, "JARH");

  const a = parsed || {};
  synth.french_text      = safeField(a.french_text,      "french_text");
  synth.grade_explique   = safeField(a.grade_explique,   "grade_explique");
  synth.isnad_chain      = safeField(a.isnad_chain,      "isnad_chain");
  synth.jarh_tadil       = safeField(a.jarh_tadil,       "jarh_tadil");
  synth.sanad_conditions = safeField(a.sanad_conditions, "sanad_conditions");
  synth.mutabaat         = safeField(a.mutabaat,         "mutabaat");
  synth.avis_savants     = safeField(a.avis_savants,     "avis_savants");
  synth.grille_albani    = safeField(a.grille_albani,    "grille_albani");
  synth.pertinence       = safeField(a.pertinence,       "pertinence");

  sseStatus(res, "HUKM");
  sseWrite(res, "hadith", { index: 0, data: synth });
  console.log("DIRECT_ANALYSIS fr:",synth.french_text.length,"isnad:",synth.isnad_chain.length);
  sseDone(res);
}

// ═══════════════════════════════════════════════════════════════════════════════
// HANDLER PRINCIPAL — SERVERLESS NODE.JS 20
// Signature : (req, res) — res.write() + res.end() — Vercel Hobby compatible
...
export default async function handler(req, res) {
  ...
  console.log("DEBUT v18.6 SERVERLESS — q:", q);

  const wantSSE = (req.headers.accept||"").includes("text/event-stream");

  if (wantSSE) {
    // ── En-têtes SSE — envoyés immédiatement (flush headers = signal de début) ──
    res.setHeader("Content-Type",      "text/event-stream; charset=utf-8");
    res.setHeader("X-Accel-Buffering", "no");
    res.setHeader("Cache-Control",     "no-cache, no-transform");
    res.setHeader("Connection",        "keep-alive");
    res.setHeader("Transfer-Encoding", "chunked");
    // ⚡ flushHeaders() — Vercel reçoit la réponse immédiatement, pas de timeout prématuré
    ...
    try {
      ...
      // ÉTAPE 2 : DORAR.NET — API new URL() (JS 2026)
      ...
      // ÉTAPE 3 : Parse ou analyse directe
      if (!html||html.length<10) {
        console.log("DORAR_EMPTY — analyse directe");
        await _doDirectAnalysis(q, arabicQuery, res);
        return;
      }

      const results=parseHadiths(html);
      if (!results.length) {
        console.log("PARSE_EMPTY — analyse directe");
        await _doDirectAnalysis(q, arabicQuery, res);
        return;
      }

      // ÉTAPE 4 : Métadonnées immédiates + analyse IA
      ...
      for (let i=0;i<results.length;i++) {
        const r=results[i];
        sseStatus(res,"RIJAL");
        const parsed=await analyserUnHadith(r,i,q,res,true);
        sseStatus(res,"JARH");
        const a=parsed||{};
      ...
      // Signal de fin propre → auto-hide barre de chargement frontend
      sseDone(res);
  ...
  // ── FALLBACK JSON classique (non-SSE) ────────────────────────────────────────
  try {
    const arabicQuery=await _translateQueryToArabic(q);
    const dorarUrlJ = new URL("https://dorar.net/dorar_api.json");
    dorarUrlJ.searchParams.set("skey", arabicQuery);
    const dorarResp=await fetchWithTimeout(
      dorarUrlJ.href,
      {headers:{"User-Agent":"Mozilla/5.0","Referer":"https://dorar.net/"}},
      9000
    ...
    const dorarData=await dorarResp.json();
    const html=dorarData?.ahadith?.result||"";
    if (!html||html.length<10) return res.status(200).json([]);
    const results=parseHadiths(html);
