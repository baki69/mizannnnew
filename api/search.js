...
// ═══════════════════════════════════════════════════════════════
// SSE HELPERS — Envoi progressif au frontend
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// analyserUnHadith — avec streaming Anthropic pour vitesse
// ═══════════════════════════════════════════════════════════════
async function analyserUnHadith(r, idx, q) {
  ...
  try {
    /* ═══ STREAMING ANTHROPIC ═══
       Au lieu d'attendre la réponse complète (~45s),
       on collecte les tokens au fur et à mesure.
       Le temps total est identique, mais on peut
       envoyer des progress events pendant la génération. */
    let rawText = "";
...
module.exports = async (req, res) => {
  ...
  try {
    // ═══ ETAPE 1 : FR→AR (PARALLÉLISÉ si Haiku nécessaire) ═══
    ...
    /* ═══ SSE : ENVOI IMMÉDIAT DES DONNÉES DORAR ═══
       Le frontend peut afficher le matn arabe + grade
       PENDANT que l'IA travaille en arrière-plan */
    if (wantSSE) {
      sseWrite(res, "dorar", results);
      sseWrite(res, "status", { step: "TAKHRIJ", msg: "Analyse par l'IA en cours...", count: results.length });
    }

    // ═══ ETAPE 4 : ANALYSE IA — PARALLÈLE + LIVRAISON PROGRESSIVE ═══
    console.log("PARALLEL_CALL: lancement", results.length, "appels");
    ...
    if (wantSSE) {
      ...
      const promises = results.map((r, i) =>
        analyserUnHadith(r, i, q).then(parsed => {
          ...
          /* Envoyer ce hadith enrichi immédiatement */
          sseWrite(res, "hadith", { index: i, data: r });
          console.log("SSE_HADITH[" + i + "] envoyé");
          return r;
        })
      );
