...
function _renderTopicList(hadiths, query) {
  ...
  if(!hadiths || hadiths.length === 0) {
    /* ── Carte "Non indexé dans Dorar" — jamais un cul-de-sac ── */
    box.innerHTML =
...
function _enrichCardSSE(idx, h) {
  ...
  requestAnimationFrame(function() {
    ...
    if(isnadSrc && isnadSrc.length > 10) {
      var cascadeHtml = _mzIsnadFromPipe(isnadSrc, gradeForPipe);
      if(cascadeHtml) {
        isnadZone.innerHTML = cascadeHtml;
        isnadZone.style.animation = 'vIn .5s ease';
      } else {
        isnadZone.innerHTML = '<div style="padding:12px 20px;font-family:Cormorant Garamond,serif;'
          + 'font-style:italic;font-size:13px;color:rgba(201,168,76,.35);">'
          + 'Cha\u00eene de transmission en cours d\u2019analyse\u2026</div>';
      }
    } else {
      isnadZone.innerHTML = '<div style="padding:16px 20px;text-align:center;">'
        + '<p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.2em;'
        + 'color:rgba(93,173,226,.3);margin-bottom:6px;">SILSILAT AL-ISN\u0100D</p>'
        + '<p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:12px;'
        + 'color:rgba(201,168,76,.25);">Analyse de la cha\u00eene en cours\u2026</p>'
        + '</div>';
    }
...
function _renderDorarCards(rawHadiths, query) {
  ...
  rawHadiths.forEach(function(r, idx) {
    ...
    html += `<div id="isnad-zone-${idx}" style="min-height:40px;transition:min-height .4s ease;position:relative;z-index:1;"></div>`;
  ...
  if (sseOK) {
    try {
      ...
      if (resp.ok && (resp.headers.get('content-type') || '').includes('text/event-stream')) {
        ...
        if (!dorarOK) _renderTopicList([], query);
        return;
      }
    } catch (sseErr) {
...
console.log('%c ✅ Mîzân v18.4 — engine.js : renderList + omniSearch + SSE chargés', 'color:#00ff00; font-weight:bold;');
