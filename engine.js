/* ═══════════════════════════════════════════════════════════════════
   ...
   Triple Bouclier :
     1. Bouclier Syntaxe  : HTML injecté via backticks uniquement
     2. Bouclier Portée   : Fonctions critiques sur window.*
     3. Bouclier Science  : Zone 2 min-height:40px + titre doré GARANTI
   CORRECTION CRITIQUE v21.0 :
     Phase 0 : Zone 1 rendue IMMÉDIATEMENT (< 100ms)
     Phase 1 : Arbre Isnad via Promise non-bloquante (rAF + setTimeout 0)
     Phase 2 : Promise.race timeout 2s → Fallback Souverain Lignée d'Or
     JSON FB  : Suppression du délai artificiel 1800ms — zéro blocage
═══════════════════════════════════════════════════════════════════ */


/* ══ BOUCLIER DE PORTÉE — Fonctions critiques sur window ══ */
window.mzAfficherArbreCanonique = window.mzAfficherArbreCanonique || function(q) {
  var box = document.getElementById('result-box');
  if(box && typeof _mzIsnadFromPipe === 'function') {
    var arbreHtml = _mzIsnadFromPipe('', 'INCONNU');
    box.innerHTML = '<div class="mz-card" style="padding:0;overflow:hidden;">' + arbreHtml + '</div>';
    box.classList.add('active');
  }
  var lb = document.getElementById('loading-box'); if(lb) lb.classList.remove('active');
};
window.goTo              = window.goTo              || goTo;
window.analyzeHadith     = window.analyzeHadith     || analyzeHadith;
window.renderList        = window.renderList        || renderList;
window.renderDetContent  = window.renderDetContent  || renderDetContent;
window.openDetail        = window.openDetail        || openDetail;
window.renderAIResult    = window.renderAIResult    || renderAIResult;
window._mzFallbackLigneeOr = window._mzFallbackLigneeOr || _mzFallbackLigneeOr;
window._mzSkeletonZone2  = window._mzSkeletonZone2  || _mzSkeletonZone2;

console.log('%c ✅ Mîzân v21.0 : Prêt pour Production', 'color: #00ff00; font-weight: bold;');
console.log('%c 🛡️ TRIPLE BOUCLIER ACTIF — Fallback Souverain connecté', 'color:#d4af37;font-weight:bold;');


/* ══ INJECTION CSS SKELETON ZONE 2 (une seule fois) ══ */
(function(){
...
/* ══ FALLBACK SOUVERAIN — LIGNÉE D'OR ══
   ...
   Déclenché automatiquement si Zone 2 vide après 2000ms             */
function _mzFallbackLigneeOr(containerId) {
  var zone = document.getElementById(containerId);
  if (!zone) return;
  /* Ne pas écraser un vrai arbre déjà rendu */
  if (zone.querySelector('.mzAr') || zone.querySelector('.mzC4-rw') || zone.querySelector('.mzAr-orb')) return;
  var GIANTS = [
    {n:'AL-BUKHARI',            d:'m. 256H',          v:'THIQAH IMAM', c:'#fde68a'},
    {n:'AHMAD IBN HANBAL',      d:'m. 241H',          v:'THIQAH IMAM', c:'#fde68a'},
    {n:'AL-ALBANI',             d:'m. 1420H',         v:'THIQAH IMAM', c:'#86efac'},
    {n:'IBN BAZ',               d:'m. 1420H',         v:'THIQAH IMAM', c:'#86efac'},
    {n:'IBN UTHAYMIN',          d:'m. 1421H',         v:'THIQAH IMAM', c:'#86efac'},
    {n:"RABI' AL-MADKHALI",     d:'hafidhahullah',    v:'THIQAH IMAM', c:'#93c5fd'}
  ];
  /* Bouclier Syntaxe : backticks */
  var h = `<div id="mz-isnad-container" style="min-height:40px;padding:14px 20px 10px;background:linear-gradient(165deg,rgba(10,6,0,.92),rgba(8,5,0,.95));">`;
  h += `<p style="font-family:Cinzel,serif;font-size:6px;letter-spacing:.3em;color:rgba(212,175,55,.55);margin-bottom:12px;text-transform:uppercase;">ZONE 2 — SILSILAT AL-ISNĀD — LIGNÉE D'OR</p>`;
  h += `<div style="display:flex;flex-direction:column;gap:6px;">`;
  GIANTS.forEach(function(g, i) {
    h += `<div style="display:flex;align-items:center;gap:10px;padding:7px 10px;background:rgba(212,175,55,.04);border:1px solid rgba(212,175,55,.12);border-radius:6px;animation:isnadFadeIn .4s ease both;animation-delay:${(i*0.08).toFixed(2)}s;">`;
    h += `<div style="width:6px;height:6px;border-radius:50%;background:${g.c};flex-shrink:0;box-shadow:0 0 8px ${g.c}60;"></div>`;
    h += `<div style="flex:1;"><p style="font-family:Cinzel,serif;font-size:8px;font-weight:700;color:${g.c};letter-spacing:.08em;">${g.n}</p>`;
    h += `<p style="font-family:'Cormorant Garamond',serif;font-size:11px;color:rgba(201,168,76,.4);">(${g.d})</p></div>`;
    h += `<span style="font-family:Cinzel,serif;font-size:6px;letter-spacing:.1em;color:${g.c};background:${g.c}15;border:1px solid ${g.c}40;border-radius:3px;padding:2px 7px;">${g.v}</span>`;
    h += `</div>`;
  });
  h += `</div>`;
  h += `<p style="font-family:Cinzel,serif;font-size:5.5px;letter-spacing:.2em;color:rgba(212,175,55,.2);text-align:center;margin-top:10px;">LIGNÉE D'OR — CHAÎNE ININTERROMPUE DE 14 SIÈCLES</p>`;
  h += `</div>`;
  zone.innerHTML = h;
...
/* Skeleton Zone 2 — HTML via backtick */
function _mzSkeletonZone2() {
  return `<div class="mz-zone2-skel"><span class="mz-zone2-skel-title">ZONE 2 — SILSILAT AL-ISNĀD — ANALYSE EN COURS…</span><div class="mz-zone2-skel-bar" style="width:78%;"></div><div class="mz-zone2-skel-bar" style="width:56%;"></div><div class="mz-zone2-skel-bar" style="width:68%;"></div></div>`;
}
...
/* ════════════════════════════════════════════════════════════════
   _enrichCardSSE v21.0 — RENDU PROGRESSIF 3 PHASES ANTI-LAG
   ● Phase 0 (immédiat, < 100ms) : Zone 1 — traduction + verdict
   ● Phase 1 (rAF + Promise)     : Calcul arbre en arrière-plan
   ● Phase 2 (timeout 2000ms)    : Fallback Souverain Lignée d'Or
   Bouclier Science : Zone 2 affiche skeleton IMMÉDIATEMENT,
   min-height:40px + titre doré garantis dès la création de la carte.
════════════════════════════════════════════════════════════════ */
function _enrichCardSSE(idx, h) {
  ...
  if(h.french) {
    ...
    if(frZone) {
      ...
      if(arEl) {
        var d=document.createElement('div');d.className='mz-matn-fr';d.innerHTML=h.french;
  ...
  var gradeForPipe = h.grade || 'DAIF';
    ...
    /* ── PHASE 1 : requestAnimationFrame — calcul arbre non-bloquant ── */
  requestAnimationFrame(function() {
    ...
    /* Skeleton immédiat — BOUCLIER SCIENCE : min-height:40px + titre doré */
    if(!isnadZone.querySelector('.mzAr') && !isnadZone.querySelector('.mzC4-rw') && !isnadZone.querySelector('.mzAr-orb') && !isnadZone.querySelector('.mz-zone2-skel')) {
      isnadZone.innerHTML = _mzSkeletonZone2();
    }

    /* Promise non-bloquante : calcul de l'arbre dans un micro-task séparé */
    var arbrePromise = new Promise(function(resolve) {
      setTimeout(function() {
        ...
        } catch(e) { console.warn('[Mîzân] arbrePromise err:', e.message); }
        resolve(result);
      }, 0);
    });

    /* PHASE 2 — Fallback Souverain déclenché si arbre pas prêt après 2000ms */
    var fallbackTimer = setTimeout(function() {
      _mzFallbackLigneeOr('isnad-zone-' + idx);
    }, 2000);

    /* Quand l'arbre est prêt, on remplace (et annule le fallback) */
    arbrePromise.then(function(cascadeHtml) {
      ...
      if(cascadeHtml) {
        ...
        /* Aucune donnée isnad → Fallback Souverain immédiat */
        _mzFallbackLigneeOr('isnad-zone-' + idx);
      }
    });

    /* PHASE 2b — accordéons Zone 2 + Zone 3 (16ms = 1 frame après Zone 1) */
    setTimeout(function() {
    /* ══ TRANCHE 3 — setTimeout 16ms : Zone 2 accordéons + Zone 3 trésor ══
       Deux frames après Zone 1 — le scroll reste fluide pendant l'injection. */
    setTimeout(function() {
      ...
      if(secAcc) {
        ...
        if(h.albani) {
          z3Html += '<details class="mz-details" style="margin:8px 18px 8px;">'
            ...
            + 'font-family:\'Cormorant Garamond\',serif;font-size:13.5px;line-height:1.8;">'
            + h.albani + '</div></details>';
        }

        if(z3Html) secAcc.innerHTML = z3Html;
      }

    }, 16); /* ~1 frame supplémentaire après la timeline */
...
function _renderDorarCards(rawHadiths, query) {
  ...
  rawHadiths.forEach(function(r, idx) {
    ...
    /* ── Zone 2 — BOUCLIER SCIENCE : skeleton immédiat, min-height:40px, titre doré ── */
    ...
    html += `</div>`; /* /zone-2-isnad — skeleton affiché immédiatement */
...
async function _searchDorarTopic(query) {
  ...
  /* ── FALLBACK JSON ─────────────────────────────────────────── */
  try {
    /* CORRECTION CRITIQUE v21.0 : suppression du délai artificiel 1800ms
       L'ancien code bloquait l'UI MINIMUM 1.8s — remplacé par fetch direct */
    var fetchResp = await fetch(searchUrl);
...
/* ════════════════════════════════════════════════════════════════════
   SILSILAT AL-ISNAD — ARBRE MAJESTUEUX v19.0
   Moteur : Arbre organique inspiré de l'image de référence
   Structure : Orbe Prophète · Tresse SVG · Bézier · Cards octogonales
   Bouclier Syntaxe : string concat sécurisée, zéro backtick HTML raw
   Mouchard : console.log vert au premier appel
════════════════════════════════════════════════════════════════════ */

/* ── Injection CSS Arbre (une seule fois) ── */
function _mzInjectArbreCSS() {
  if (document.getElementById('mzArbre-css')) return;
  /* Les styles .mzAr-* sont dans style.css — on injecte uniquement
     les @keyframes d'animation dynamique ici pour garantie */
...
/* ══════════════════════════════════════════════════════════════════
   _mzIsnadFromPipe v19.0 — ARBRE MAJESTUEUX
   Entrée : isnadChain (string pipe-séparé) + grade (SAHIH/DAIF…)
   Sortie : HTML string de l'arbre complet
══════════════════════════════════════════════════════════════════ */
function _mzIsnadFromPipe(isnadChain, grade) {
  ...
  if (!window._mzArbre_loaded) {
    console.log('%c \u2705 M\u00eezan v19.0 : _mzIsnadFromPipe (Arbre Majestueux) charg\u00e9', 'color:#00ff00;font-weight:bold;');
  ...
  /* ── DONNÉES FIXES 19 PLAQUES (image de référence) ── */
  /* Ces 6 rangs représentent la chaîne canonique de l'Isnad
     Chaque rang : { lft:[], ctr:{}, rgt:[] }
     lft/rgt = mini-nœuds latéraux, ctr = carte centrale principale */
  var LIGNEE = [
    {
      cta: 'a transmis \u00e0',
      lft: [{ n:'MUHAMMAD IBN ISMAIL\nAL-BUKHARI', d:'m. 256H', r:'Imam, Mouhaddith, Fakih\nSahih Al-Bukhari', v:'THIQAH', nc:'#d4af37' }],
      ctr: { n:'AHMAD IBN\nHANBAL', d:'m. 241H', r:'Imam, Mouhaddith, Fakih\nImam, Thiqah', v:'THIQAH', nc:'#fde68a', bc:'rgba(251,191,36,.09)', bd:'rgba(251,191,36,.5)' },
      rgt: []
    },
    {
      cta: 'ont transmis aux',
      lft: [{ n:"BARI' AL-MADKHALI", d:'m. 1min.', r:'Fakih, Mouhaddith\nAdil', v:'MUDALLIS', nc:'#fca5a5' }],
      ctr: { n:'AL-NAWAWI', d:'m. 676H', r:'Imam\nMouhaddith, Adil', v:'ADIL', nc:'#d4af37', bc:'rgba(212,175,55,.08)', bd:'rgba(212,175,55,.42)' },
      rgt: [{ n:'IBN HAJAR\nAL-ASQALANI', d:'n. 853H', r:'Hafidh\nMouhadditheen, Adil', v:'ADIL', nc:'#86efac' }]
    },
    {
      cta: 'ont transmis aux',
      lft: [{ n:'IBN AL-QASIM', d:'m. Ant.', r:'Fakih\nMouhaddith', v:'ADIL', nc:'#86efac' }],
      ctr: { n:'IBN AL-QAYYIM', d:'m. 751H', r:'Fakih\nMouhaddith, Adil', v:'ADIL', nc:'#d4af37', bc:'rgba(212,175,55,.08)', bd:'rgba(212,175,55,.42)' },
      rgt: [{ n:'IBN HAJAR\nAL-ASQALANI', d:'m. 852H', r:'Hafidh\nMouhaddith, Thiqah', v:'THIQAH', nc:'#86efac' }]
    },
    {
      cta: 'ont transmis aux',
      lft: [{ n:"RABI' AL-MADKHALI", d:'5h. Jarh', r:'Fakih, Mouhaddith\nAdil', v:'THIQAH', nc:'#86efac' }],
      ctr: { n:'IBN HAJAR', d:'m. 852H', r:'Hafidh\nMouhaddith, Adil', v:'THIQAH', nc:'#93c5fd', bc:'rgba(96,165,250,.08)', bd:'rgba(96,165,250,.42)' },
      rgt: [
        { n:'IBN HAJAR\nAL-ASQALANI', d:'m. 852H', r:'Mouhaddith\nThiqah', v:'THIQAH', nc:'#86efac' },
        { n:"RABI' AL-MADKHALI", d:'n. Fakih', r:'Prane\nMohd., Adil', v:'MUDALLIS', nc:'#fca5a5' }
      ]
    },
    {
      cta: 'ont transmis aux',
      lft: [{ n:'IBN AL-SHENIN', d:'m. 65H', r:'Fakin\nMoncar', v:'MUDALLIS', nc:'#fca5a5' }],
      ctr: { n:'IBN HAJAR\nAL-ASQALANI', d:'m. 852H', r:'Mouhaddith\nThiqah', v:'THIQAH', nc:'#93c5fd', bc:'rgba(96,165,250,.08)', bd:'rgba(96,165,250,.42)' },
      rgt: [{ n:'AL-AL-QYIM', d:'m. 751H', r:'Fakih\nMohacash', v:'MUNKAR', nc:'#fca5a5' }]
    },
    {
      cta: 'ont transmis aux',
      lft: [{ n:"RABI' AL-MADKHALI", d:'m. 1445H', r:'Mouhaddith, Adil', v:'THIQAH', nc:'#86efac' }],
      ctr: { n:"RABI' AL-MADKHALI", d:'m. 1445H', r:'Mouhaddith, Adil', v:'THIQAH', nc:'#86efac', bc:'rgba(34,197,94,.09)', bd:'rgba(34,197,94,.45)' },
      rgt: [{ n:"RABI' AL-MADKHALI", d:'m. 1445H', r:'Mouhaddith, Adil', v:'THIQAH', nc:'#86efac' }]
    }
  ];

  /* ── MERGE données dynamiques si présentes ── */
  if (dynNodes.length >= 3) {
    /* Remplacer les cartes centrales par les nœuds dynamiques parsés */
    for (var di = 0; di < Math.min(dynNodes.length, LIGNEE.length); di++) {
      var dn = dynNodes[di];
      var vis = nodeVis(dn.titre, dn.verdict, di, dynNodes.length);
      LIGNEE[di].ctr.n  = dn.nom;
      LIGNEE[di].ctr.nc = vis.nameC;
      LIGNEE[di].ctr.bd = vis.dotC + '60';
      LIGNEE[di].ctr.bc = vis.dotC + '12';
      LIGNEE[di].ctr.v  = dn.verdict || LIGNEE[di].ctr.v;
      if (dn.titre)  LIGNEE[di].ctr.r = dn.titre;
      if (dn.siecle) LIGNEE[di].ctr.d = dn.siecle;
    }
  }

  /* ── HELPER : classe badge verdict ── */
  ...
  /* ── HELPER : card centrale HTML ── */
  function _renderCtr(d, delay) {
    var dlStr = delay.toFixed(2) + 's';
    var h = '<div class="mzAr-col-c">';
    h += '<div class="mzAr-crd" style="animation-delay:' + dlStr + ';border-color:' + (d.bd || 'rgba(212,175,55,.48)') + ';background:linear-gradient(145deg,' + (d.bc ? d.bc : 'rgba(22,16,4,.97)') + ',rgba(12,8,0,.99));" onclick="window.mzOpenIsnadPanel(\'' + _esc(d.n) + '\',\'' + _esc(d.r || '') + '\',\'' + _esc(d.v || '') + '\',\'' + _esc(d.d || '') + '\',\'' + (d.nc || '#d4af37') + '\')">';
  ...
  /* ── HELPER : échappement sécurisé pour attributs onclick ── */
  function _esc(s) {
    return (s || '').replace(/\\/g,'\\\\').replace(/'/g,"\\'").replace(/"/g,'&quot;').replace(/\n/g,' ');
  }

  /* ── HELPER : SVG branche Bézier gauche ── */
  function _svgBranchLeft(nh) {
    /* Courbe partant du centre vers la gauche */
    return '<svg class="mzAr-branch" width="28" height="' + (nh || 56) + '" viewBox="0 0 28 56" style="flex-shrink:0;">'
      + '<defs><linearGradient id="mzBg" x1="1" y1="0" x2="0" y2="0">'
      + '<stop offset="0%" stop-color="rgba(212,175,55,.55)"/>'
      + '<stop offset="100%" stop-color="rgba(212,175,55,.08)"/></linearGradient></defs>'
      + '<path d="M28,28 C20,28 10,22 2,14" stroke="url(#mzBg)" stroke-width="2" fill="none" stroke-linecap="round"/>'
      + '<path d="M28,28 C20,28 10,34 2,42" stroke="url(#mzBg)" stroke-width="1" fill="none" stroke-linecap="round" opacity=".4"/>'
      + '</svg>';
  }

  /* ── HELPER : SVG branche Bézier droite ── */
  function _svgBranchRight(nh) {
    return '<svg class="mzAr-branch" width="28" height="' + (nh || 56) + '" viewBox="0 0 28 56" style="flex-shrink:0;">'
      + '<defs><linearGradient id="mzBg2" x1="0" y1="0" x2="1" y2="0">'
      + '<stop offset="0%" stop-color="rgba(212,175,55,.55)"/>'
      + '<stop offset="100%" stop-color="rgba(212,175,55,.08)"/></linearGradient></defs>'
      + '<path d="M0,28 C8,28 18,22 26,14" stroke="url(#mzBg2)" stroke-width="2" fill="none" stroke-linecap="round"/>'
      + '<path d="M0,28 C8,28 18,34 26,42" stroke="url(#mzBg2)" stroke-width="1" fill="none" stroke-linecap="round" opacity=".4"/>'
      + '</svg>';
  }

  /* ── HELPER : SVG tronc tressé (fixe, traverse tout l'arbre) ── */
  function _svgTrunk(totalRows) {
    var totalH = 120 + totalRows * 140 + 60;
    var s = '<svg class="mzAr-trunk" height="' + totalH + '" viewBox="0 0 72 ' + totalH + '" style="top:0;height:' + totalH + 'px;">';
    s += '<defs>';
    s += '<linearGradient id="mzTkGd" x1="0" y1="0" x2="0" y2="1">';
    s += '<stop offset="0%" stop-color="rgba(255,220,80,.7)"/>';
    s += '<stop offset="40%" stop-color="rgba(180,140,30,.4)"/>';
    s += '<stop offset="100%" stop-color="rgba(212,175,55,.6)"/>';
    s += '</linearGradient>';
    s += '<filter id="mzGlw"><feGaussianBlur stdDeviation="2" result="b"/>';
    s += '<feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>';
    s += '</defs>';
    /* Tresse — 3 brins entrelacés */
    var y0 = 110, y1 = totalH;
    /* Brin central */
    s += '<path d="M36,' + y0 + ' C28,' + (y0+60) + ' 44,' + (y0+120) + ' 36,' + (y0+180) + ' C28,' + (y0+240) + ' 44,' + (y0+300) + ' 36,' + y1 + '" stroke="url(#mzTkGd)" stroke-width="7" fill="none" stroke-linecap="round" filter="url(#mzGlw)" opacity=".8"/>';
    /* Brin gauche */
    s += '<path d="M28,' + y0 + ' C20,' + (y0+60) + ' 32,' + (y0+120) + ' 24,' + (y0+180) + ' C16,' + (y0+240) + ' 28,' + (y0+300) + ' 22,' + y1 + '" stroke="rgba(180,140,30,.5)" stroke-width="3.5" fill="none" stroke-linecap="round" opacity=".7"/>';
    /* Brin droit */
    s += '<path d="M44,' + y0 + ' C52,' + (y0+60) + ' 40,' + (y0+120) + ' 48,' + (y0+180) + ' C56,' + (y0+240) + ' 44,' + (y0+300) + ' 50,' + y1 + '" stroke="rgba(160,120,20,.45)" stroke-width="3" fill="none" stroke-linecap="round" opacity=".65"/>';
    /* Maillons de chaîne dorée (cercles décoratifs) */
    var chainY = y0 + 40;
    while (chainY < y1 - 20) {
      s += '<ellipse cx="36" cy="' + chainY + '" rx="5" ry="3" stroke="rgba(212,175,55,.55)" stroke-width="1.2" fill="none"/>';
      chainY += 32;
    }
    s += '</svg>';
    return s;
  ...
  /* ── HELPER : SVG racines organiques ── */
  function _svgRoots() {
    return '<svg class="mzAr-roots" viewBox="0 0 340 44" preserveAspectRatio="xMidYMid meet">'
      + '<path d="M170 0 C150 10,115 25,80 38" stroke="rgba(212,175,55,.22)" stroke-width="2.5" fill="none"/>'
      + '<path d="M170 0 C155 12,145 26,140 44" stroke="rgba(212,175,55,.18)" stroke-width="2" fill="none"/>'
      + '<path d="M170 0 C170 12,170 26,170 44" stroke="rgba(212,175,55,.28)" stroke-width="3" fill="none"/>'
      + '<path d="M170 0 C185 12,195 26,200 44" stroke="rgba(212,175,55,.18)" stroke-width="2" fill="none"/>'
      + '<path d="M170 0 C190 10,225 25,260 38" stroke="rgba(212,175,55,.22)" stroke-width="2.5" fill="none"/>'
      + '<path d="M170 0 C140 14,100 30,65 44" stroke="rgba(212,175,55,.14)" stroke-width="1.5" fill="none"/>'
      + '<path d="M170 0 C200 14,240 30,275 44" stroke="rgba(212,175,55,.14)" stroke-width="1.5" fill="none"/>'
      + '</svg>';
  }
  ...
  try {
    ...
    /* Titre doré */
    h += '<p class="mzAr-title">ZONE 2 \u2014 SILSILAT AL-ISN\u0100D \u2014 14 SI\u00c8CLES</p>';
    ...
    /* Connecteur initial */
    h += '<div class="mzAr-link"><div class="mzAr-link-pipe" style="height:20px;"></div><span class="mzAr-link-badge">a transmis \u00e0</span><div class="mzAr-link-pipe" style="height:8px;"></div></div>';
    ...
    LIGNEE.forEach(function(rang, ri) {
      ...
      /* Branche Bézier gauche */
      h += _svgBranchLeft();

      /* Carte centrale */
      h += _renderCtr(rang.ctr, delay + 0.06);

      /* Branche Bézier droite */
      h += _svgBranchRight();
      ...
      if (ri < LIGNEE.length - 1) {
        ...
        h += '<div class="mzAr-link"><div class="mzAr-link-pipe" style="height:14px;"></div><span class="mzAr-link-badge">' + nextCta + '</span><div class="mzAr-link-pipe" style="height:8px;"></div></div>';
      }
    ...
    console.error('%c \ud83d\udd34 v19.0 _mzIsnadFromPipe CRASH', 'color:#ff0000;font-weight:bold;', err);
...
function renderList(){
  ...
  GROUPS.forEach(function(grp){
    ...
    if(items.length>visibleItems.length){
      ...
      ch+='<button onclick="iaSectionLoadMore(\''+secId+'\');renderList();" '
